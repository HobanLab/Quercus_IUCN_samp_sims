...
#Finally, there is a flag to turn on calculation of allele categories--all_cat_flag. This code will calculcate
#The alleles in several categories (ie., global, global very common, global rare, etc...)
...
#This flag allows you to calculate the allele categories 
#If TRUE, calculcate allele categories 
all_cat_flag = TRUE
...
#these lead to the directories that the simulation files are stored in
...
final_quercus_results = array(0, dim = c(max_sample_size,num_replicates,num_species))

final_alleles_all_quercus = array(0, dim=c(max_sample_size,num_replicates,num_species))
#################################################################################
#FST VARIABLES
#storing Fst results
...
#only testing Fst for first 100 replicates (instead of 1000) since it is very time consuming to run, and unlikely to change much with increased replicates
mean_max_min_fst = array(dim = c(3,100,num_species))#Fst run on 100 replicates
...
for(i in 1:length(species_list)) {
  setwd(paste(mydir,species_list[i],sep="")) #getting the directory holding the current species simulation files
  list_files = list.files(paste(mydir,species_list[i],sep=""), pattern = ".gen$") #make a list of that species
  for(j in 1:(length(list_files))) { 
    ...
    if(all_cat_flag == TRUE) {
      ...
      #Allele category code calculating/saving 
      if(all_cat_flag == TRUE) {
